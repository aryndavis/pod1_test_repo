...
result = sp.search(name, type = 'artist', limit = 1)
...
artist_id = sp.artist(result['artists']['items'][0]['id'])
print(artist_id) # uncomment if you want to see the results


print("Question 4")
print()
# How popular is Beyonce? Using her unique artist ID, now let's pull popularity and followers
#
...
artist_info = (artist_id["followers"]['total'])
jprint(f"{name} has a total follower count of {artist_info}")


# How do you to access key-value pairs for popularity?
jprint(f"{name} has a popularity score of {artist_id['popularity']}")
...
chosen_artists = ['Kendrick Lamar', 'Nipsey Hussle', 'Eminem', 'Jay-Z', 'slowthai']
popularities = []


for artists in chosen_artists:
    artist_cat = sp.search(artists, type = 'artist', limit = 1)
    #jprint(artist_cat)
    artist_name = (artist_cat['artists']['items'][0]['name'])
    #print(artist_name)
    artist_ids = (artist_cat['artists']['items'][0]['id'])
    #jprint(artist_ids)
    artist_infos = sp.artist(artist_ids)
    #jprint(artist_infos)
...
In this loop I learned that you must append to a new list (not the list I am iterating through) 
because it will infinitely loop. I must append to a new list.
...
def popularity_info():
    for artist in popularities:
    #jprint(artist)
        artist_names = (artist['Artist'])
        #jprint(artist_names)
        artist_pop = (artist['Artist Popularity'])
        #jprint(artist_pop)
        artist_follow = (artist['Artist followers'])
        #jprint(artist_follow)
        print(f"{artist_names} has a {artist_pop} popularity score with {artist_follow} followers.")
    return (popularity_info)
# Were we supposed to arrange this from highest to lowest or vice versa?
popularity_info()
...
sim =  sp.artist_related_artists(popularities[0]['Artist ID']) # fill this in
sim = (sim['artists'][:3]) # uncomment this like to see what the top 3 similar artists are
#jprint(sim)
...
for artists in sim:
    ...
    #jprint(artists)
    new_names = (artists['name'])
    #jprint(new_names)
    new_pop = (artists['popularity'])
    #jprint(new_pop)
    print(f"The related artist is {new_names} with a respective {new_pop} popularity score.")
