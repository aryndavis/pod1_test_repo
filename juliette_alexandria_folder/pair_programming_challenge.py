...
def withdraw(account, amount):
    if account['balance'] > amount:
        ...
        print(f"Your current balance is {account['balance']} and there are not enough funds to withdraw your requested amount.")

withdraw(my_acct, 20.15)
#print(my_acct)
withdraw(my_acct, 10.30)
#print(my_acct)
withdraw(my_acct, 50.00)
#print(my_acct)
...
def deposit_secure(account, amount):
    ...
    password = input('Create a password: ')
    balance = (float(0))
    if username == user_acct_info["user_name"] and password == user_acct_info["pass_word"]:
        print(f"Depositing {balance} to your account.")
        break
    else:
        print("Username/password combination incorrect. Try again.")

deposit_secure(user_acct_info, 100)
