...
print("The data type is a list, because it is encompassed in hard brackets and when you print(type(books)) you will find out its a list")
...
print("The first data type that I see is a list, and nested in the list is dictionaries, inside each dictionary are several keys and values that tells you title, author and a description of the books. I know it is a list because the variable is assigned to a value that has a hard bracket. Inside the hard bracket, we have dictionaries, inside the dictionaries are key and value pairs.")
...
def search_by_author(author):
    blank_list = [] # creating an emply list to store the titles later
    for book in books_with_details: # loops through all the dictionary
        if book["author"] == author: # check each dictionaries author to see if it matches the argument
            blank_list.append(book["title"]) #if the argument matches, find the title in that dictionary and add it to the list
    return(blank_list) # once the list is filled and completed return the list to the function
