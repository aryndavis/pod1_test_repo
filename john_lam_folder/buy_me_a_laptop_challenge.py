...
brands = ", ".join(all_laptops)

# TODO: Write code to print all laptop names
print(f"All laptops names: {brands}")
...
fix_prices = [str(int) for int in prices]
str_of_ints = ",".join(fix_prices)

print(f"The prices of the 16-inch MacBook Pro are {str_of_ints}")

# TODO: Write code to print all the color options for Dell XPS 13.

color = laptops[1]["types"][0]["colors"]
color_scheme = [str(int) for int in color]
what = ", ".join(color_scheme)

print(f"The color option for Dell XPS 13 are: {what}")
...
print("Suppose that the 16-inch MacBook Pro in space gray color is sold out. Also, the same laptop with 1 TB storage is out of stock as well. Update the list of dictionaries such that these options are removed. Print the updated dictionary.")

# TODO: Update the laptops dictionary.
remove = laptops[0]["types"][1]["colors"].remove("space gray")
take_away = laptops[0]["types"][1]["storage"].remove("1 TB SSD")
...
for run_down in laptops:
    for i in run_down["types"]:
        for p in i["price"]:
            print(p)
